...

{
    ...
    public class DBDefs
    {
        ...
        public bool ConnectToDB(string server, string username, string pass, string database)
        {
            ...
            if (!Server.Equals(String.Empty))
            {
                ...
                catch (Exception Ex)
                {
                    ...
                    string ErrorMessage = "SQL Server veritabanýna baðlanýrken hata ile karþýlaþýldý.";
                    ErrorMessage += Environment.NewLine;
                    ErrorMessage += Environment.NewLine;
                    ErrorMessage += Ex.Message;

                    MessageBox.Show(ErrorMessage, "Baðlantý Hatasý", MessageBoxButtons.OK, MessageBoxIcon.Error);
            ...
            {
                MessageBox.Show("Veritabaný Ýsmi Belirtilmemiþ!", "Baðlantý Hatasý", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
        }
        ...
        {
            if (cbShowLog != null)
                cbShowLog(true, "Veritabaný", msg, 8);
        ...
        public int ExecuteNonQuery(String Sql, out string msg)
        {
            ...
            if (SQLConnection != null)
            {
                if (SQLConnection.State == System.Data.ConnectionState.Open)
                {
                    ...
                    {
                        msg = "SQL Hatasý : " + ex.Message + " (Sorgu:" + Sql + ")";
                        SqlInAction = false;
                        GiveError(msg);
                        return -1;
                    }
        ...
        public object ExecuteScalar(String Sql)
        {
            if (SQLConnection != null)
            {
                if (SQLConnection.State == System.Data.ConnectionState.Open)
                {
                    ...
                    {
                        GiveError(ex.Message + " Sql:" + Sql);
                        MessageBox.Show(ex.Message + "\n\nSorgu:" + Sql, "Sorgu Yürütme Hatasý", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return null;
                    }
                    catch (Exception ex)
                    {
            ...
            if (SQLConnection != null)
            {
                if (SQLConnection.State == System.Data.ConnectionState.Open)
                {
                    ...
                    catch (SqlException ex)
                    {
                        ...
                        MessageBox.Show(ex.Message, "DataTable Hatasý", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return null;
                    }
                    catch (Exception ex)
                    {
            ...
            if (SQLConnection != null)
            {
                if (SQLConnection.State == System.Data.ConnectionState.Open)
                {
                    ...
                    catch (SqlException ex)
                    {
                        ...
                        MessageBox.Show("Sorgu:" + Sql + "\r\n\n" + ex.Message, "Dataset Hatasý", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return null;
                    }
                    catch (Exception ex)
                    {
            ...
            if (SQLConnection != null)
            {
                if (SQLConnection.State == System.Data.ConnectionState.Open)
                {
                    ...
                    catch (SqlException ex)
                    {
                        ...
                        MessageBox.Show(ex.Message, "Veri Okuyucu (Data Reader) Hatasý", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return null;
                    }
                    catch (Exception ex)
                    {
